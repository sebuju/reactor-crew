...
//!
//! Reader/compare code is copied verbatim from the stage probes (frozen;
//! source noted per fn). If a stage format drifts, the embedded asserts
//! fail loudly here — update the copy from the named source.
use sim_rs::step::*;
use sim_rs::tick::*;
use sim_rs::{core, ctl, events, field, room, sec, store, tick, transport};
...
impl Cmp {
    fn fail(&mut self, si: usize, msg: String) {
        self.fails += 1;
    ...
    fn sdig(&mut self, si: usize, path: &str, a: f64, b: f64) {
    ...
    fn exact_u8(&mut self, si: usize, path: &str, a: u8, b: u8) {
        if a != b {
            self.fail(si, format!("{path} {a} vs {b}"));
        }
    }
    fn exact_bool(&mut self, si: usize, path: &str, a: bool, b: bool) {
        if a != b {
            self.fail(si, format!("{path} {a} vs {b}"));
        }
    }
    fn exact_i32(&mut self, si: usize, path: &str, a: i32, b: i32) {
        if a != b {
            self.fail(si, format!("{path} {a} vs {b}"));
        }
    }
    fn exact_u32(&mut self, si: usize, path: &str, a: u32, b: u32) {
        if a != b {
            self.fail(si, format!("{path} {a} vs {b}"));
        }
    }
    fn exact_str(&mut self, si: usize, path: &str, a: &str, b: &str) {
        if a != b {
            self.fail(si, format!("{path} {a} vs {b}"));
        }
    }
...
fn read_smap(c: &mut Cur) -> sec::SMap {
...
fn cmp_smap(cmp: &mut Cmp, si: usize, path: &str, a: &sec::SMap, b: &sec::SMap) {
...
fn read_sec_state(c: &mut Cur) -> sec::SecState {
...
fn cmp_map_f64(cmp: &mut Cmp, si: usize, path: &str, a: &HashMap<String, f64>, b: &HashMap<String, f64>) {
...
fn cmp_map_bool(cmp: &mut Cmp, si: usize, path: &str, a: &HashMap<String, bool>, b: &HashMap<String, bool>) {
    let mut ka: Vec<&String> = a.keys().collect();
    let mut kb: Vec<&String> = b.keys().collect();
    ka.sort();
    kb.sort();
...
fn cmp_map_str(cmp: &mut Cmp, si: usize, path: &str, a: &HashMap<String, String>, b: &HashMap<String, String>) {
    let mut ka: Vec<&String> = a.keys().collect();
    let mut kb: Vec<&String> = b.keys().collect();
    ka.sort();
    kb.sort();
...
fn cmp_vec_str(cmp: &mut Cmp, si: usize, path: &str, a: &[String], b: &[String]) {
    if a != b {
        cmp.fail(si, format!("{path} {a:?} vs {b:?}"));
    }
}

fn cmp_vec_f64(cmp: &mut Cmp, si: usize, path: &str, a: &[f64], b: &[f64]) {
    if a.len() != b.len() {
...
fn cmp_sec_state(cmp: &mut Cmp, si: usize, tag: &str, a: &sec::SecState, b: &sec::SecState) {
    let p = tag;
...
fn read_room_smap(c: &mut Cur) -> room::SMap {
    let n = c.u32() as usize;
    let keys = c.strs(n);
    let vals = c.f64a(n);
    room::SMap { keys, vals }
...
fn cmp_room_smap(cmp: &mut Cmp, si: usize, path: &str, a: &room::SMap, b: &room::SMap) {
    let mut ka: Vec<&String> = a.keys.iter().collect();
    let mut kb: Vec<&String> = b.keys.iter().collect();
    ka.sort();
    kb.sort();
...
fn read_dmg_core(c: &mut Cur) -> DmgCore {
    let breach = c.u8() != 0;
...
fn read_room_state(c: &mut Cur) -> room::RoomState {
    let mut f64s = HashMap::new();
    for _ in 0..c.u32() {
        let k = c.str();
        let v = c.f64();
...
fn cmp_dmg_core(cmp: &mut Cmp, si: usize, path: &str, a: &DmgCore, b: &DmgCore) {
...
fn cmp_room_state(cmp: &mut Cmp, si: usize, tag: &str, a: &room::RoomState, b: &room::RoomState) {
    let p = tag;
    cmp_map_f64(cmp, si, &format!("{p}.f64s"), &a.f64s, &b.f64s);
    cmp_map_bool(cmp, si, &format!("{p}.u8s"), &a.u8s, &b.u8s);
...
fn read_ev_vessel(c: &mut Cur) -> events::EvVessel {
    let n = c.f64();
    let decay = c.f64();
    let dmg = c.f64();
    let melt_frac = c.f64();
...
fn read_map_u8(c: &mut Cur) -> HashMap<String, u8> {
    let mut m = HashMap::new();
    for _ in 0..c.u32() {
        let k = c.str();
        let v = c.u8();
...
fn read_f64map(c: &mut Cur) -> HashMap<String, f64> {
    let n = c.u32() as usize;
...
fn read_events_state(c: &mut Cur) -> events::EventsState {
    let tick = c.i32();
    let n = c.f64();
    let decay = c.f64();
    let heat = c.f64();
...
fn cmp_ev_vessel(cmp: &mut Cmp, si: usize, path: &str, a: &events::EvVessel, b: &events::EvVessel) {
...
fn cmp_map_u8(cmp: &mut Cmp, si: usize, path: &str, a: &HashMap<String, u8>, b: &HashMap<String, u8>) {
    let mut ka: Vec<&String> = a.keys().collect();
    let mut kb: Vec<&String> = b.keys().collect();
    ka.sort();
    kb.sort();
...
fn cmp_events_state(cmp: &mut Cmp, si: usize, tag: &str, a: &events::EventsState, b: &events::EventsState) {
    let p = tag;
...
fn read_core_state(c: &mut Cur) -> core::CoreState {
    let phi = c.f64an();
    let x_i = c.f64an();
    let x_x = c.f64an();
    let n_tf = c.f64an();
...
fn cmp_core_state(cmp: &mut Cmp, si: usize, tag: &str, a: &core::CoreState, b: &core::CoreState) {
    let p = tag;
...
use sim_rs::room::{ByRun, FireCool, FireFull, RoleRow, RoomDer, RoomMeta, RoomPart};

// RoomMeta reader: copied verbatim from room-probe.rs read_meta (gate
// writes via room-gate.js dumpMeta).
fn read_room_meta(c: &mut Cur) -> RoomMeta {
    let gw = c.u32() as usize;
    let gh = c.u32() as usize;
    let mpc = c.f64();
    let pcont = c.f64();
...
fn read_sec_curves(c: &mut Cur) -> sec::SecCurves {
    let n = c.u32() as usize;
    let mut curves = vec![];
    for _ in 0..n {
...
fn read_room_curves(c: &mut Cur) -> room::RoomCurves {
    let n = c.u32() as usize;
    if std::env::var("PROBE_DEBUG").is_ok() {
...
fn read_core_k(c: &mut Cur) -> core::CoreK {
    use sim_rs::core::XNR;
...
use sim_rs::events::{EvHaz, EvPart, EvRun, EvSteamBook, EvTank, EventsMeta};

// EventsMeta reader: copied verbatim from events-probe.rs read_meta (gate
// writes via events-gate.js dumpMeta).
fn read_events_meta(c: &mut Cur) -> EventsMeta {
    let gw = c.u32() as usize;
    let gh = c.u32() as usize;
...
fn read_solve_frozen(c: &mut Cur) -> SolveFrozen {
    let n = c.u32() as usize;
...
fn read_trans_meta(c: &mut Cur) -> TransMeta {
    let n = c.u32() as usize;
    let ne = c.u32() as usize;
    let eu = c.u32a(ne);
    let ev = c.u32a(ne);
...
use sim_rs::ctl::{Act, Block, CoreAct, ReliefCell, Sample, SINK_UNKNOWN};

// CtlMeta reader: 6 sink key lists (S0-once; gate dumps in dump order).
fn read_ctl_meta(c: &mut Cur) -> CtlMeta {
    CtlMeta {
        freg_keys: c.strsn(),
        flow_keys: c.strsn(),
        valve_keys: c.strsn(),
...
fn read_ctl_sample(c: &mut Cur) -> (Sample, Vec<f64>, Vec<f64>) {
    let dt = c.f64();
    let live = c.u8() != 0;
    let n = c.u32() as usize;
...
fn read_solve_tail(c: &mut Cur, n: usize, ne: usize, n_loops: usize, nt: usize, ncv: usize) -> SolveTail {
...
fn read_sec_tail(c: &mut Cur, meta: &SecMeta) -> SecTail {
...
fn read_trans_tail(c: &mut Cur, n: usize, ne: usize, ntc: usize, nr: usize) -> TransTail {
    TransTail {
        src: c.f64a(n),
        metal_qv: c.f64a(n),
        metal_qm: c.u8a(n),
...
fn read_core_tail(c: &mut Cur) -> CoreTail {
    CoreTail {
        h_in: c.f64(),
        sink: c.u8() != 0,
        sat: c.f64(),
...
fn read_tube_tail(c: &mut Cur) -> TubeTail {
    let taken = c.u8() != 0;
    if !taken {
...
fn read_events_tail(c: &mut Cur) -> EventsTail {
    EventsTail {
        rps_state: c.str(),
        sink_runback: c.u8() != 0,
        runback_live: c.u8() != 0,
...
// __STAGE_READERS__
fn cmp_bag(cmp: &mut Cmp, si: usize, path: &str, a_v: &[f64], a_h: &[u8], b_v: &[f64], b_h: &[u8]) {
...
fn read_log_list(c: &mut Cur) -> Vec<(u8, u32)> {
    let n = c.u32() as usize;
    let mut v = Vec::with_capacity(n);
    for _ in 0..n {
        v.push((c.u8(), c.u32()));
...
fn main() {
    ...
    for pi in 0..np {
        ...
        let sec_meta = read_sec_meta(&mut c);
        if std::env::var("PROBE_DEBUG").is_ok() {
            eprintln!("preset {pi} post-secmeta o={}", c.o);
        }
        let sec_curves = read_sec_curves(&mut c);
        let room_meta = read_room_meta(&mut c);
        if std::env::var("PROBE_DEBUG").is_ok() {
            eprintln!("preset {pi} post-roommeta o={}", c.o);
        }
        let room_curves = read_room_curves(&mut c);
        let events_meta = read_events_meta(&mut c);
        if std::env::var("PROBE_DEBUG").is_ok() {
        ...
        for _ in 0..ncore {
            let id = c.str();
            core_k.insert(id.clone(), read_core_k(&mut c));
            core_ids.push(id);
        }
        if std::env::var("PROBE_DEBUG").is_ok() {
            eprintln!("preset {pi} post-coreK o={}", c.o);
        }
        let solve_frozen = read_solve_frozen(&mut c);
        if std::env::var("PROBE_DEBUG").is_ok() {
        ...
        let trans_meta = read_trans_meta(&mut c);
        if std::env::var("PROBE_DEBUG").is_ok() {
        ...
        // ---- S0: states ----
        let sec0 = read_sec_state(&mut c);
        if std::env::var("PROBE_DEBUG").is_ok() {
            eprintln!("preset {pi} S0-sec o={}", c.o);
        }
        let room0 = read_room_state(&mut c);
        if std::env::var("PROBE_DEBUG").is_ok() {
            eprintln!("preset {pi} S0-room o={}", c.o);
        }
        let events0 = read_events_state(&mut c);
        if std::env::var("PROBE_DEBUG").is_ok() {
        ...
        for _ in 0..ncore2 {
            let id = c.str();
            core0.insert(id, read_core_state(&mut c));
        }
        if std::env::var("PROBE_DEBUG").is_ok() {
        ...
        // solve carried init: F seed (13) + warr + fixV + choke
        let n = meta.solve.n;
        ...
        let log0init = read_log_list(&mut c);
        let tick0 = c.u32();
        if std::env::var("PROBE_DEBUG").is_ok() {
        ...
        // canonical mass/dmg init from sec dump (equal everywhere at S0)
        ...
        for ti in 0..nticks {
            ...
            let _tag = c.u32();
            let dt = c.f64();
            // ctl sample (+want out/f) + per-tick key lists
            let (sample, want_out, want_f) = read_ctl_sample(&mut c);
            if std::env::var("PROBE_DEBUG").is_ok() {
            ...
            // (read_ctl_meta S0 section removed; keys ride per tick)
            ...
            // out_pre/f_pre must equal carried (sync check); cold S0 has no
            // blk yet (commission never ran ctlPass; gate ensure-fills zeros
            // before tick0) so adopt pre as carried instead of failing.
            if sample.out_pre.len() == st.blk_out.len() {
                for (i, (a, b)) in sample.out_pre.iter().zip(st.blk_out.iter()).enumerate() {
                    if a.to_bits() != b.to_bits() && !(a.is_nan() && b.is_nan()) {
                        cmp.fail(si, format!("blk_out pre drift [{i}] {a} vs {b}"));
                        break;
                    }
                }
            } else if st.blk_out.is_empty() && st.blk_f.is_empty() {
                st.blk_out = sample.out_pre.clone();
                st.blk_f = sample.f_pre.clone();
            } else {
                cmp.fail(si, format!("blk_out len {} vs {}", sample.out_pre.len(), st.blk_out.len()));
            }
            // tails
            let solve_tail = read_solve_tail(&mut c, n, ne, meta.solve.n_loops, meta.solve.tank_order.len(), meta.solve.cond_v.len());
            if std::env::var("PROBE_DEBUG").is_ok() {
                eprintln!("tick {ti} solve-tail o={}", c.o);
            }
            let sec_tail = read_sec_tail(&mut c, &meta.sec);
            if std::env::var("PROBE_DEBUG").is_ok() {
                eprintln!("tick {ti} sec-tail o={}", c.o);
            }
            let trans_tail = read_trans_tail(&mut c, n, ne, meta.trans.tavg_circs.len(), meta.trans.rise_lo.len());
            let mut core_tail = HashMap::new();
            for _ in 0..ncore {
                let id = c.str();
                core_tail.insert(id, read_core_tail(&mut c));
            }
            ...
            let events_tail = read_events_tail(&mut c);
            if std::env::var("PROBE_DEBUG").is_ok() {
            ...
            for _ in 0..ntube {
                let id = c.str();
                tube.insert(id, read_tube_tail(&mut c));
            }
            // post states
            if std::env::var("PROBE_DEBUG").is_ok() {
                eprintln!("tick {ti} pre-postsec o={}", c.o);
            }
            let want_sec = read_sec_state(&mut c);
            if std::env::var("PROBE_DEBUG").is_ok() {
                eprintln!("tick {ti} post-sec o={}", c.o);
            }
            let want_room = read_room_state(&mut c);
            if std::env::var("PROBE_DEBUG").is_ok() {
            ...
            let want_liq = c.u32();
            let want_pgen = c.u32();
            let want_events = read_events_state(&mut c);
            ...
            for _ in 0..ncore_post {
                let id = c.str();
                want_core.insert(id.clone(), read_core_state(&mut c));
            }
            ...
            // solve carried post: F(13) memo(5) warr
            let wf = c.f64a(n);
            let wf1 = c.f64a(n);
            let wf2 = c.f64a(n);
            let wf3 = c.f64a(n);
            ...
            let wm4 = c.f64a(n);
            let wwarr = c.f64a(ne);
            let want_log = read_log_list(&mut c);
            ...
            let f0 = cmp.fails;
            st.tick += 1;
            let r = step_replay(&meta, &mut st, &tick);
            // === compare ===
...
fn st_mass_out_init() -> HashMap<String, f64> {
    HashMap::new()
}
