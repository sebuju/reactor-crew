"use strict";
/* commissioning's rest state, against the shell's mass balance, steady continuity, stored mass at rest and each pump's suction off the saturation line; again: the settle is a function of the plant */
const {check, watch, psat, if97} = require("../lib.js");
const rest = (G, pre) => {
  ...
  { const snap = G.engSnap(G.engSnapNew()), ci = PT.coreCirc0, pn = PT.circPNode[ci];
    ...
    const tap = () => { G.engRestore(snap); G.eNetInvalidate(); ST.sc[G.SC_DICEOFF] = 1;
      watch(G, {cap:1}); return G.eLoopP(ci)/P.P0; };
    const clear = r => r > 0.935 && r < 1.05, r = tap();
    ...
    if(name === "CALDER HALL"){
      ...
      const tr = watch(G, {cap:10, event:() => ST.sc[G.SC_SCRAMMED] ? "scram" : ""}).t;
...
const again = (G, pre) => {
  ...
  settle(c0); P.snap0 = snap0;
};
module.exports = {rest, again};
