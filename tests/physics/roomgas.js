...
const G = ["pocket", "fill", "evict", "cavity", "swell", "lock", "slug"].includes(mode) ? require("./lib.js").load() : commissionPreset(mode === "nafire" ? 3 : 0);
let ST = G.ST;
...
const ciQ = ["pocket","fill","evict","cavity","swell","lock","slug"].includes(mode) ? -1 : quietCell();
...
function drive(n){
  let res = 0, neg = 0;
  watch(G, {cap:n*0.02, each:() => { res += ST.sc[G.SC_FACERES];
...
function pocketAudit(dt){
  ...
  return {
    ...
    post(pDrive){
      ...
      let n = 0, b1 = 0, l1 = -1, b2 = 0, q = 0;
      const st = [];
      for(let s=0;s<N;s++){ if(lab[s] >= 0 || !gas(s)) continue;
        st.push(q); let h = q; lab[s] = n; Q[q++] = s;
        while(h < q){ const i = Q[h++], X = i%GW;
          const js = [X < GW-1 && bx[i] ? i + 1 : -1, X > 0 && bx[i-1] ? i - 1 : -1, i < N-GW && by[i] ? i + GW : -1, i >= GW && by[i-GW] ? i - GW : -1];
          for(const j of js) if(j >= 0 && lab[j] < 0 && gas(j)){ lab[j] = n; Q[q++] = j; } }
        n++; }
      st.push(q);
      ...
      for(let r=0;r<n;r++){ const dV = V0[r] - V1[r]; if(!(dV > 0)) continue;
        ...
        for(let u=st[r];u<st[r+1];u++){ const i = Q[u]; cond += gm0[i] - ST.roomM[i];
...
if(mode === "evict"){
  ...
  const note = "cell " + (i%GW) + "," + Y1 + (flooded ? " flooded, empty" : " NOT a flooded empty cell") + ", air " + T.toFixed(1) + " K at " + (p/1000).toFixed(2) + " kPa";
...
if(mode === "cavity"){
  ...
  const T0 = 500, X0 = 20, X1 = 28, Y0 = 14, Y1 = 20, ps = psat(T0);
  ...
  let E0 = 0, v0 = 0, Tp = [], P0 = [];
  watch(G, {cap:0.2, step:() => { E0 = watE(); v0 = vap(); Tp = cells.map(i => ST.roomWater[i] > 0 ? Tw(i) : 0); P0 = cells.map(i => ST.roomWP[i]); G.step(0.02); },
    each:k => {
      ...
      if(k === 1){ T1 = Tw(c); dE = watE() - E0; mv = vap() - v0; hg = if97r2(psat(T1), T1).h; cells.forEach((i, j) => { vdp += wv(i)*(ST.roomWP[i] - P0[j]); }); }
      if(k === 2 && ST.roomM[c] > 0){ const want = psat(T1); e2 = Math.abs(pAbs(c) - want)/want; } }});
  const SAT = "IAPWS-IF97 region 4: a cavity in water holds its own vapour at p_sat(T) of the water around it";
  const note = "cell " + (c%GW) + "," + Y0 + ", water at " + Tw(c).toFixed(1) + " K, p_sat " + psat(Tw(c)).toFixed(3) + " MPa";
...
if(mode === "swell"){
  ...
  let sum = 0, n = 0;
  watch(G, {cap:3, each:k => { if(k > 50){ sum += b0.V - gasOf(bell).V; n++; } }});
  const got = sum/n;
  const note = "swell " + dV.toFixed(4) + " m3 of " + wet.length + " cells heated " + T0 + " -> " + T1 + " K; pocket " + b0.V.toFixed(2) + " m3, outer air " + o0.V.toFixed(2) + " m3, surfaces " + Ain.toFixed(2) + " and " + Aout.toFixed(2) + " m2; the pocket's loss averaged over 1-3 s";
...
if(mode === "lock"){
  ...
  const lockCase = (tag, pre) => {
    ...
    check(tag + "every cell's occupancy and tightness is what the drawing puts there", occBad, 0, 0,
      ...
      {abs:true, unit:"cells", note:occAt || "all " + G.GW*G.GH + " cells agree"});
    watch(G, {cap:0.1});
    ...
    check(tag + "a sealed full body given q at constant volume: its mean pressure rise", got, want, 0.02,
      ...
      {unit:"MPa", note:lock.length + " cells after 0.5 s"});
    check(tag + "...and its mean temperature rise", gT, wT, 0.02,
      ...
      {unit:"K", note:lock.length + " cells after 0.5 s"});
    let fit = 0, dp = 0, fAt = "";
    ...
    check(tag + "a sealed full body heated 2 K in place holds the pressure its water fits the box at", fit, 0, 1 - G.LIQ_FULL_K,
      ...
      {abs:true, unit:"relative", note:"worst of " + lock.length + " cells after 0.5 s, " + fAt + "; mean pressure " + dp.toFixed(3) + " MPa over 1 atm"}); };
...
if(mode === "slug"){
  ...
  watch(G, {cap:3, step:() => { aud.pre(); G.step(0.02); }, each:(k, t) => { eCount(En, aud.post(0), t);
    const b = gasOf(pocket);
    if(b.V > 0) cumW += 0.5*(pprev + b.p)*(Vprev - b.V);
    const pb = ST.roomP[resCell] + G.ROOM_P0;
    drvW += 0.5*(pbPrev + pb)*(Vprev - b.V);
    if(b.p > pmax){ pmax = b.p; tPeak = t; Wpeak = cumW; drvPeak = drvW; mPeak = b.m; }
  ...
  check("slug: peak pocket pressure against Bagnold's air spring", pmax, pm, gridTol, BAGNOLD,
    {unit:"kPa", note:"analytic peak " + (pm/1000).toFixed(3) + " MPa (in the 0.3-3 MPa window), xm " + xm.toFixed(3) + " m; tolerance the pm spread moving xm half a cell (" + (100*gridTol).toFixed(1) + " %)"});
  check("slug: compression work never exceeds the slug's kinetic energy plus the drive's work", Wpeak, KE + drvPeak, 0.02, BAGNOLD,
    {unit:"kJ", pass:Wpeak <= (KE + drvPeak)*1.02, note:"work to the first peak " + Wpeak.toFixed(2) + " kJ against KE " + KE.toFixed(2) + " kJ plus back-gas work " + drvPeak.toFixed(2) + " kJ; friction may only lose energy"});
  check("slug: per pocket per tick, the work on the gas within the drive's work and the water's energy", En.bad, 0, 0, ENERGY,
    {abs:true, unit:"pocket-ticks", note:eNote(En)});
...
/* a break watched until its room pressure peak has passed by one transit of the loop it drains, the blast window is over and the trip is answered */
if(mode === "breakhl" || mode === "breaksl"){
  const cut = mode === "breakhl" ? "pipe:28,15" : "pipe:35,4", what = mode === "breakhl" ? "hot-leg" : "steam-line";
  const HORIZON = 60, sc = ST.sc;
  ...
  const win = transit(G);
  G.actId("hit", cut);
  const A = {pmax:0, pAt:"", tMax:0, src:0, clamp:0, blast:"", bw:0, bwAt:"", fmax:0, fAt:"", fT:300, tScr:-1, rod0:0, rod1:-1, e:eTally()};
  // liquid past its bubble point, not yet boiled: saturated liquid at its own temperature
  const TofHf = h => rootUp(T => if97(psat(T), T).h, h, 273.16, 623.15);
  ...
  const w = watch(G, {cap:3*win, horizon:HORIZON, window:win, step:() => { aud.pre(); G.step(0.02); }, each:(k, t) => { tNow = t;
    ...
    for(let i=0;i<N;i++){ const p = (ST.roomP[i] + G.ROOM_P0)/1000;
      ...
      if(wk !== cW[i] || we !== cE[i] || wp !== cP[i] || pool !== cL[i]){
        ...
        cW[i] = wk; cE[i] = we; cP[i] = wp; cL[i] = pool; cT[i] = T; cF[i] = (wk*(wk > 0 ? if97(hot ? psat(T) : pw, T).v : 0) + pool)/V0; }
      if(cF[i] > A.fmax){ A.fmax = cF[i]; A.fT = cT[i]; A.fAt = cellAt(i); } }
    ...
    event:t => t > 1 && A.rod1 >= 0 && t - A.tMax >= win ? "room peak " + A.pmax.toFixed(3) + " MPa at " + A.tMax.toFixed(2) + " s passed" : ""});
  const run = watchNote(w) + " of a " + HORIZON + " s question, window " + win.toFixed(2) + " s";
  check(what + " break: per pocket per tick, the work on the gas within the drive's work and the water's energy", A.e.bad, 0, 0, ENERGY,
    {abs:true, unit:"pocket-ticks", pass:A.src > 0 && A.e.bad === 0, note:eNote(A.e) + "; highest cell of the run " + A.pmax.toFixed(2) + " MPa, " + A.pAt + ", against a highest drive of " + A.src.toFixed(2) + " MPa; " + run});
  check(what + " break: cell-ticks at the ROOM_TMAX guard", A.clamp, 0, 0,
  ...
  check(what + " break: the fullest cell's water and pool over its volume", A.fmax, 1, tol,
  ...
  if(mode === "breaksl") check("steam-line break: scram demanded after the break, rods inserting", A.tScr, 0, 2,
    SLB_SRC, {abs:true, unit:"s", pass:A.tScr >= 0 && A.tScr <= 2 && ins,
      note:A.tScr < 0 ? "never scrammed in " + w.t.toFixed(2) + " s" : "rods " + A.rod0.toFixed(3) + " -> " + A.rod1.toFixed(3) + " one second after the demand"});
    ...
    {abs:true, unit:"scrammed", pass:A.tScr >= 0 && ins, note:A.tScr < 0 ? "never scrammed in " + w.t.toFixed(2) + " s" : "demanded " + A.tScr.toFixed(2) + " s after the break"});
}

if(mode === "flash"){
  // the split through its own door: a water node's h let go into a cell held at p
  const PT = G.PT, i = ci, P = [0.101325, 0.2, 0.5], H = [500, 1000, 1500, 2000];
  let nd = -1; for(let k=0;k<PT.n.node && nd < 0;k++) if(PT.sats[PT.nodeSat[k]] === G.SAT_WATER) nd = k;
  const h0 = ST.hBy[nd], p0 = ST.roomP[i];
  let worst = 0, at = "", rows = [];
  for(let a=0;a<P.length;a++) for(const h of H){ const p = P[a];
    ST.hBy[nd] = h; ST.roomP[i] = p*1000 - G.ROOM_P0;
  ...
  check("flash split of water let go into a room, on IF97", worst, 0, 0.01,
    ...
    {abs:true, unit:"quality", pass:nd >= 0 && worst <= 0.01, note:"worst " + at + "; model/IF97 " + rows.join(", ")});
}
...
if(mode === "nafire"){
  ...
  const run = () => { const S = G.engSnap(G.engSnapNew());
    ...
    watch(G, {cap:1, step:() => { src.fill(0); G.eFireStep(0.02, src); }});
