"use strict";
// chunks: relay box
/* plan-reactor-ui 6.2: a bench redraw that grows the vessel under routed pipework (relay), and every preset's vessel box in cells over MPC
   against the fixed 9x12 MACHINE.core and the board (box); the stock vessel against the four-loop reference is plant.js */
...
if(mode === "box"){
  ...
  for(let pre=0;pre<G.PLANTPRE.length;pre++){ G.plantPreset(pre); G.buildLayout();
