...
/* a run takes one shot at its middle cell: what is measured is losing the connection, not the length of it. */
test("hit every machine, port and pipe run on the stock PWR", async ({ page }) => {
  ...
  const targets = await page.evaluate(() => {
    /* combatHit() refuses a shield and an unfitted part, and a declined target is fifty unchanged ticks. */
    ...
    for (const key of Object.keys(M.byKey).sort()) {
      ...
      /* a run between two port-adjacent machines spans no cell to aim at. */
