...
function planStats(d){ return [
  ...
  ["MODERATOR COEFF",d.aM.toFixed(0)+(d.aG?" / "+d.aG.toFixed(0):"")+" pcm/K",clamp(-(d.aM+d.aG)/70,0,1),d.aM+d.aG>0?C.red:C.blue,
  ...
  ["STUCK-ROD MARGIN",(d.sdmStuck>0?"+":"")+d.sdmStuck.toFixed(0)+" pcm",clamp(d.sdmStuck/2000,0,1),d.sdmStuck<200?C.red:C.green,
   "The shutdown margin with the most worthy single rod cluster stuck out, as a licence states it: the bank margin less that cluster's rest-flux worth ("+d.rodW.toFixed(0)+" pcm), the rest on the curve. The core has no azimuth, so a cluster is priced smeared round its ring. Under 200 pcm the design does not shut down with one rod failed."],
...
const LATREAD=[
  ...
  ["MODERATOR COEFF",cD=>{ const d=derived(coreIdOf(cD)); return d.aM.toFixed(0)+(d.aG?" / "+d.aG.toFixed(0):"")+" pcm/K"; },
   "What heating the moderator is worth: the coolant over its share of the moderation, and any blocks you packed over theirs, each at its own material's coefficient; a second figure is blocks with a temperature of their own, which follow power slowly. It is the negative feedback that makes the plant follow load by itself."],
