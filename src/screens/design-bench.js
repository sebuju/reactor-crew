...
function planStats(d){ return [
  ...
  ["SHUTDOWN MARGIN",d.sdm.toFixed(0)+" pcm",clamp(d.sdm/2000,0,1),d.sdm<200?C.red:C.green,
   "How firmly a full scram holds the core down once it cools and the xenon decays. Below about 200 pcm the reactor creeps back to power after a trip."],
  ["THERMAL MARGIN",d.dnbr.toFixed(2)+" DNBR",clamp((d.dnbr-1)/2.5,0,1),d.dnbr<1.4?C.amber:C.green,
   "Headroom before coolant boils into an insulating film on the fuel. Sets your real overload ceiling, not the power rating."],
