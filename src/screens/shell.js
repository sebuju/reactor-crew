...
/* ═══════════════ SCREENS ═══════════════ */
let screen="design",H=790,helpScroll=0,helpMax=0;
function layout(){ H = screen==="design"?H||1400 : screen==="operate"?H||1700 : 700; resize(); }
function resize(){
  const cssW=Math.min(Math.max(740,scroller.clientWidth),820), sc=cssW/W, dpr=devicePixelRatio||1;
...
function topbar(){
  ...
  for(const [k,label] of tabs){
    const w=tw(label,{size:9,sp:1.8,caps:1})+22, on=screen===k,
          dis=k==="operate"&&!P, isHelp=k==="help";
    const wd=push({x,y:9,w,h:23,type:"btn",fn:()=>{if(!dis){screen=k;helpScroll=0;layout();}}});
    ...
    TIP(x,9,w,23,label,
      ...
      : k==="operate" ? (dis?"Locked until you commission a reactor on the design bench.":"The live control room. Run the plant, push it past its limits, and repair it when it bites back.")
  ...
  txt(P?`${P.id}  ${pad(P.rated,4)} MWt`:"NO CORE COMMISSIONED",748,20,
      {size:9,sp:1,align:"right",color:P?C.cyan:C.amber});
  txt(clk,748,31,{size:9,sp:1,align:"right",color:C.ink2});
  if(Math.floor(performance.now()/500)%2) fillRect(748-tw(clk,{size:9,sp:1})-11,24,6,7,C.amber);
}
