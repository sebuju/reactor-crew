...
const HELP=[
 ...
 ["d","COMPARTMENT LAYERS","Heat is a place, the same way radiation is, and these three are the survey for it. AIR TEMP bands the air temperature in every cell, from AMBIENT to UNTENABLE, with the band edges set at the temperatures machines actually give up at rather than at round numbers. H2 CLOUD shows where hydrogen off the cladding has ended up once it has left the loop, and reddens at the 4 per cent flammability limit. PART TEMP prints, on each machine, what it is standing in against what it was built for. Unlike the radiation layers these need a running plant, so the bench skips them. What puts heat into the room is every hot surface, and - far more - anything discharging steam or water into the compartment instead of into a tank. What takes it out is the hull, and whatever ventilation you paid for."],
 ["h","DESIGN BENCH"],
