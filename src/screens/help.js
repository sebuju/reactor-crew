...
const HELP=[
 ...
 ["d","BLOCK VALVE / HPI","Both worked on the diagram. Every relief valve carries its own two-switch strip: BLOCK shuts a valve that will not reseat, and AUTO/BYP decides whether that one valve may lift by itself. HPI refills the loop and adds vessel fatigue the entire time it runs."],
 ["d","FITTINGS","A fitting is a tap on a pipe, not a machine in a cell. Right-click any run at the design bench to put one on it: a JUNCTION ties two runs together, a THROTTLE chokes one down, and a RELIEF VALVE vents to the relief tank. It costs a spool piece and its own branch pipe, and it moves with the run it taps - move a machine upstream and the tap comes with it. Every fitting has its own panel in both rails, and clicking its symbol on the diagram brings that panel up."],
