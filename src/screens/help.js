...
const HELP=[
 ...
 ["d","FILL LEVELS","The shaded column inside the pressurizer and steam generator follows loop inventory. The vessel's is the real level, drawn to scale off its own geometry: lower plenum, core and upper plenum, drained from the top. Its water is tinted by its own temperature in the pipes' colours, and over the level it goes dark under a cyan surface line. A red frame round the fuel means thermal margin is lost."],
 ...
 ["d","RADIATION LAYERS","Four switches under the LAYERS key on the head row, beside the zoom key, on both screens. RAD ZONES fills the floor in five bands, from CLEAR through EXCLUSION, and draws the boundary line where a shield's shadow actually falls. CELL DOSE prints the exact number in every free cell. REPAIR CELLS outlines just the cells a body could physically stand in beside something. PART DOSE prints, on the machine itself, what reaching it will cost. This is the survey: a machine is drawn over the field, and the cells left visible under it are the cells you can actually send someone to stand in."],
 ["d","COMPARTMENT LAYERS","Heat is a place, the same way radiation is, and these are the survey for it. HEAT bands the air temperature in every cell, from AMBIENT to UNTENABLE, with the band edges set at the temperatures machines actually give up at rather than at round numbers, and prints on each machine its own skin temperature against what it was built for and the kilowatts it is putting into the room. H2 CLOUD shows where hydrogen off the cladding has ended up once it has left the loop, and reddens at the 4 per cent flammability limit. Unlike the radiation layers these need a running plant, so the bench skips them. What puts heat into the room is every hot surface, and - far more - anything discharging steam or water into the compartment instead of into a tank. What takes it out is the hull, and whatever ventilation you paid for."],
 ["h","DESIGN BENCH"],
 ...
 ["d","FUEL TEMP","Hottest pellet temperature anywhere in the core, driving damage. Past the damage temperature of the fuel you picked, it takes damage regardless of DNBR. FUEL MEAN beside it is the flux-weighted average, and that is what the protection trips on."],
 ["h","CONTROLS"],
