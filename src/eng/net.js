...
const E_NS_TURBWK=0, E_NS_TURBWKP=1, E_NS_TURBWKA=2, E_NS_QSGT=3, E_NS_SPILL=4, E_NS_NAT=5,
      E_NS_CORE=6, E_NS_NPIECE=7, E_NS_NF=8, E_NS_BW=9, E_NS_REFINE=10, E_NS_FACTORS=11, E_NS_TURBWKH=12, E_NS_N=13;
...
function eNetReadEdges(){
  ...
  let core = 0, spill = 0, wk = 0, wkp = 0, wka = 0, wkh = 0, qs = 0;
  for(let e=0;e<E;e++){
    ...
    if(PT.edWork[e]){ const b = PT.edTurb[e], fr = b >= 0 ? ST.turbWorkFr[b] : 0;
      if(fr > 0){ const w = qe*fr, aw = Math.abs(w); wk += w; wkp += x[u]*aw; wka += aw; eNodeHOfA(ST.pBy, qe >= 0 ? u : v); wkh += E_NH[0]*aw; } }
  ...
  const S = SX.netSc;
  S[E_NS_TURBWK] = wk; S[E_NS_TURBWKP] = wkp; S[E_NS_TURBWKA] = wka; S[E_NS_TURBWKH] = wkh; S[E_NS_QSGT] = qs;
