...
const E_CO_DOP=0, E_CO_MOD=1, E_CO_EXP=2, E_CO_VD=3, E_CO_XE=4, E_CO_ROD=5, E_CO_TIP=6, E_CO_DIS=7, E_CO_H2=8, E_CO_FCI=9, E_CO_N=10;
...
function eRodShape(c){
  const nb = c*XNN, rb = c*XNR, bb = c*PT.nbMax, NB = PT.coreNB[c], rinf = PT.coreRinf[c], tipLen = PT.coreTipLen[c];
  const cov = ST.csNCov, fol = ST.csNFol;
  for(let k=0;k<XNN;k++){ cov[nb+k] = 0; fol[nb+k] = 0; }
  for(let b=0;b<NB;b++){
    const ins = clamp(ST.csRodZ[bb+b], 0, 1), tip = XNZ*(1 - ins), fLo = tip - tipLen, fHi = tip, br = PT.coreBankR[bb+b];
    for(let i=0;i<XNR;i++){
...
function eCoreSeed(c, x0, n0){
  ...
  s.csTubesOpen[c] = 0; s.csCavRelief[c] = 0;
  ...
  s.csDecay[c] = d; s.csHeat[c] = n0*PROMPT_F + d;
}
...
function eCoreReset(c, flowNet){
  ...
  for(let k=0;k<XNN;k++){ const i = nb + k;
    ...
    s.csNDmg[i] = 0; s.csNOx[i] = 0; s.csNOxI[i] = 0; s.csNMelt[i] = 0; s.csNDisp[i] = 0; s.csNDnb[i] = 0; }
...
/* E_MN = [heat, pw, rise, Tin, Tf, gShare, x, dhSub] in, margin out at [8]: per node, so no double crosses as an argument */
const E_MN = new Float64Array(9);
function eMarginNode(c){
  const heat = E_MN[0], pw = E_MN[1], rise = E_MN[2], Tin = E_MN[3], Tf = E_MN[4], gShare = E_MN[5], x = E_MN[6], dhSub = E_MN[7];
  const law = PT.coreDnbLaw[c], K = PT.coreDnbrK[c];
  const q = heat*PT.coreRated[c]*1e6/Math.max(PT.coreAHeat[c], 1e-6)*Math.max(pw, 1e-3);
  if(law === E_DNB_BOIL){ E_MN[8] = K*(dhSub/PT.coreCp[c])/Math.max(rise, 1e-3); return; }
  if(law === E_DNB_TEMP){ E_MN[8] = K*Math.max(PT.coreTdmg[c] - Tin, 0)/Math.max(Tf - Tin, 1e-3); return; }
  ...
  const Q = q > 1 ? q : 1;
  if(gSI0 >= gFloor){ E_MN[8] = K*w/Q; return; }
  E_CHF[0] = pMPa; eChfZuberA(); const z = E_CHF[4];
  E_MN[8] = K*Math.min(w, z + (w - z)*gSI0/gFloor)/Q;
}
...
const E_CS = new Float64Array(7), E_RV = new Float64Array(5), E_CMX = new Float64Array(MX_N);
function eCoreStep(c){
  ...
  for(let i=0;i<XNR;i++){
    ...
    const dTn = heat*dT0*mixK[i]/(XNZ*ff*chan);
    ...
    for(let j=0;j<XNZ;j++){
      ...
      const dh = cp*dTn*pw, hMid = h + dh/2; h += dh;
      ...
      s.csNVt[k] = E_VQ[2];
      E_MN[0] = heat; E_MN[1] = pw; E_MN[2] = hMid/cp - Tcold; E_MN[3] = Tcold; E_MN[4] = s.csNTf[k];
      E_MN[5] = mflux*chan; E_MN[6] = xe; E_MN[7] = dhSub; eMarginNode(c);
      const dnb = E_MN[8];
      if(dnb < dnbLo){ dnbLo = dnb; dnbK = q; }
      const hCsp = film0*bare/cladR;
      const hCnb = qhat*pw*bare/Math.max(sat + JL_K*Math.pow(Math.max(qpp0*q2, 1)/1e6, 0.25)*Math.exp(-pCore/JL_P) - s.csNTc[k], 1e-3);
      ...
      oxP += qOx*nodeW[q];
      const qPin = qhat*pw*(1 - s.csNDisp[k]);
      let Tn = dt > 0 ? s.csNTf[k] + (qPin + qOx - film*(s.csNTf[k] - s.csNTc[k]))*dt/tauF
                      : s.csNTc[k] + (qPin + qOx)/Math.max(film, 1e-9);
  ...
  { const node = T.coreNode[c], m = node >= 0 ? s.mBy[node] : E_NAN, V = node >= 0 ? T.nodeVol[node] : 0;
    let rhoIn;
  ...
  if(W2 > 0) for(let q=0;q<=E_CO_DIS;q++) o[q] /= W2;
  eNodePeak(c);
...
function eCorePRead(){
  const s = ST, n = PT.n.core;
  for(let c=0;c<n;c++){ const i = PT.coreNode[c], p = i >= 0 ? s.pBy[i] : E_NAN;
...
function eCoreVesselStep(dt){
  const s = ST, n = PT.n.core;
  for(let c=0;c<n;c++){
    const S0 = PT.coreSat[c], pc = s.csPCore[c], node = PT.coreNode[c], rv = E_RV;
    rv[0] = pc; satTA(S0, rv, 0, 1);
    const sat = rv[1];
    const m = node >= 0 ? s.mBy[node] : E_NAN, kg0 = PT.coreKg0[c];
    let vLeak = 0;
    ...
    if(o[E_CO_H2] > 0 && node >= 0 && m > DRY_MIN_KG) s.h2By[node] += o[E_CO_H2]/m;
...
function eCoreMeltStep(dt){
  ...
  for(let c=0;c<nc;c++){
    ...
    if(s.csMeltFrac[c] > 0 && !P.catcher){
      const node = PT.coreNode[c], have = node >= 0 ? s.mBy[node] : E_NAN;
      const want = E_MELT_INV*s.csMeltFrac[c]*dt/100*PK[PK_INVKG0];
      const kg = have === have ? Math.min(want, Math.max(have, 0)) : 0;
...
function eCoreSeal(){
  const n = PT.n.core;
  for(let c=0;c<n;c++){ const i = PT.coreNode[c], m = i >= 0 ? ST.mBy[i] : E_NAN;
