...
const LAYERS={
  ...
  roomc:{group:"COMPARTMENT", label:"CORIUM",     seam:"env",   data:"room", live:true, on:true,
        draw:roomCorLayer,
        tip:"Molten core that has left the vessel. It pours straight down under the vessel to the first thing that holds it, spreads along that floor while it is liquid, and eats the concrete under it and beside it - AMBER is melt still liquid, dull red is crust. The dark bite under a cell is the concrete it has eaten, with its depth: the basemat goes in a day or so, a painted concrete wall when the bite reaches its thickness. A core catcher under the vessel holds it on a cooled iron floor. It paints nothing until the vessel has failed, which is why it ships on."},
