"use strict";
// node tools/server.js [port] [--live] - the optional save server
...
// a kind is half a filename: this list is a whitelist, not a hint
const KINDS = ["scenarios", "recordings"];

const MAXBODY = 32 * 1024 * 1024;
const MAXSNAP = 192 * 1024 * 1024;

// the one place untrusted input becomes a filename: whitelist, so no separator or dot reaches path.join()
const IDPAT = /^[A-Za-z0-9_-]{1,64}$/;
// a dump carries its own extension, so a dot is allowed - but never a LEADING one, or `..` is back
...
// deliberately not the schema: that lives on the browser side and a copy here would drift
...
function listKind(kind){
  let names = [];
  try{ names = fs.readdirSync(dirFor(kind)); }catch(e){ return []; }
  return names.filter(f => f.endsWith(".json"))
              .map(f => f.slice(0, -5))
              .filter(id => IDPAT.test(id))
              .sort()
...
function snapApi(req, res, parts){
  if(parts.length === 1){
    if(req.method === "GET"){
      ...
      // newest first: the name carries the timestamp
...
// the traversal guard tests the RESOLVED path; `+ path.sep` matters or a sibling `reactor-crew-secrets` passes
