...
/* the room geometry is compiled at commission, so a wall this probe paints goes on before it */
const utube = mode === 'utube' || mode === 'utubegas', uy0 = mode === 'utubegas' ? 1 : 25;
let wx = 0;
if (utube) {
  for (const x of [26, 23, 22, 33, 34]) { let ok = true; for (let Y = uy0; Y <= 28; Y++) if (!M.matPaint(x, Y)) ok = false; if (ok) { wx = x; break; } for (let Y = uy0; Y <= 28; Y++) M.matLift(x, Y); }
  M.buildLayout();
}
M.commission();
const s = M.ST(), sc = s.sc; sc[SC_DICEOFF] = 1;
const GW = M.GW(), GH = M.GH(), N = GW * GH, MPC = M.MPC(), at = (x, y) => y * GW + x, g = M.G_MPA() * 1e6;
const W = () => s.roomWater, water = () => { M.eLqBind(); return M.E_LQ()[0]; }, metal = () => { M.eLqBind(); return M.E_LQ()[1]; };
const G = () => { M.eRoomLive(); const P = M.PT(); return {tight:P.rTight, occ:P.rOcc, own:P.rOwn, pan:P.rPan}; };
const liqSpeed = (q, i) => { const X = i % GW; let v = 0;
  if (X < GW - 1) v = Math.max(v, Math.abs(q.vu[i]));
  if (X > 0) v = Math.max(v, Math.abs(q.vu[i - 1]));
  if (i < N - GW) v = Math.max(v, Math.abs(q.vv[i]));
...
function dump(q, y0, y1) {
  ...
  for (let Y = y0; Y <= y1; Y++) {
    let r = String(Y).padStart(2) + ' ';
    for (let X = 0; X < GW; X++) {
      const i = at(X, Y), m = q.M[i], c = M.eLqCap(q, i), f = Math.min(9, Math.floor(10 * m / Math.max(c, 1e-9)));
      r += M.eLqShut(i) ? '#' : !(m > 0.01) ? '.' : m > 1.01 * c ? '*' : M.eLqStands(q, i) ? String(f) : 'abcdefghij'[f];
    }
    console.log(r);
  }
}
...
function surf(x0, x1) {
  ...
  for (let X = x0; X <= x1; X++) { let z = null;
    for (let Y = GH - 1; Y >= 0; Y--) { const i = at(X, Y); if (W()[i] > 0.5 && M.eLqStands(q, i) && !M.eLqShut(i)) z = M.eLqSurf(q, i); }
...
function pool(x0, x1, yFloor, depthM, T) {
  ...
  for (let Y = yFloor; Y > yFloor - Math.ceil(rows); Y--) {
    ...
    for (let X = x0; X <= x1; X++) { const i = at(X, Y); if (M.eLqShut(i)) continue;
      const kg = f * M.eLqCap(q, i); M.eLiqLandAt(q, i, kg, kg * e, 0); }
  }
  // what the lay buried over its own cap, read before the first tick can take it
  const over = [];
  for (let i = 0; i < N; i++) { const ex = W()[i] - M.eLqCap(q, i);
    if (ex > 1 && i >= GW && W()[i - GW] > 0 && !M.eLqShut(i - GW)) over.push([i % GW, (i / GW) | 0, ex]); }
  ...
  for (const F of [s.roomM, s.roomH2, s.roomO2, s.roomVap]) { let tot = 0, vol = 0;
    for (let Y = 1; Y <= 29; Y++) for (let X = 7; X <= 36; X++) { const i = at(X, Y); tot += F[i]; vol += M.eRoomVgas(i); }
    for (let Y = 1; Y <= 29; Y++) for (let X = 7; X <= 36; X++) { const i = at(X, Y); F[i] = tot * M.eRoomVgas(i) / vol; } }
  tick(1);
  for (let Y = yFloor; Y > yFloor - Math.ceil(rows); Y--) for (let X = x0; X <= x1; X++) { const i = at(X, Y);
    if (W()[i] >= M.eLqCap(q, i) - 1e-6) s.roomWP[i] = s.roomP[i] + M.WATER_RHO() * g * (depthM - (yFloor - Y) * MPC) / 1000; }
  return over;
}
...
function flips(k, label, every) {
  ...
  let tot = 0, worst = 0;
  const read = () => { const Gg = G(), now = new Uint8Array(N); for (let i = 0; i < N; i++) now[i] = W()[i] > 10 ? (M.eLqStands(q, i) ? 1 : 2) : 0; return now; };
  let a = read();
...
if (mode === 'map') { map(); }
  ...
  const z1 = surf(7, 36);
  console.log('surface span ' + (span(z1) * 1000).toFixed(1) + ' mm (laid ' + (span(z0) * 1000).toFixed(1) + '), ms/tick ' + (msAcc / msN).toFixed(3) + ', liquid solve it ' + sc[SC_LIQCGIT]);
  ...
  for (let i = 0; i < N; i++) if (W()[i] > 0) { if (M.eLqStands(q, i)) stand += W()[i]; else air += W()[i]; vmax = Math.max(vmax, liqSpeed(q, i)); }
  ...
  console.log('wall at x ' + wx + ', rows ' + uy0 + '-28');
  if (mode === 'utubegas') pool(7, 36, 29, 3 * MPC); else { pool(7, 36, 29, MPC); pool(7, wx - 1, 28, 2 * MPC); }
  if (mode === 'utubegas') { M.actId('injectOn', 'o2', 50, at(wx + 1, 20)); tick(5); M.act('injectOff'); }
  ...
  for (let j = 0; j < 200; j++) { tick(1);
    for (let i = 0; i < N; i++) { const d = s.roomM[i] / M.eRoomVgas(i) / (M.ROOM_M0() / M.ROOM_VCELL()); if (d > worst) worst = d; }
    if (top === null && W()[b] > 0.9 * M.eLqCap(q, b) && W()[at(25, 27)] > 0.9 * M.eLqCap(q, at(25, 27))) top = t; }
  ...
  const wet = tick(200);
  console.log('dry ' + dry.toFixed(2) + ' ms/tick, 10 t/s pour ' + wet.toFixed(2) + ' ms/tick, liquid solve it ' + sc[SC_LIQCGIT] + ', gas ' + sc[SC_ROOMCGIT]);
  verdict(wet < 8, 'the pour case under 8 ms a tick');
} else if (mode === 'deck') {
  M.actId('injectOn', 'fluid', 10000, at(29, 13));
  for (let j = 0; j < 8; j++) { if (j === 4) M.act('injectOff'); const ms = tick(50); const z = surf(7, 36);
    console.log('t ' + t.toFixed(0) + ' sump ' + (wTot() / 1000).toFixed(3) + ' t, surface ' + (z.length ? f3(Math.min(...z)) + '-' + f3(Math.max(...z)) : '-') + ' over ' + z.length + ' cols, res ' + sc[SC_MASSRES].toFixed(3) + ', ms ' + ms.toFixed(2) + ', it ' + sc[SC_LIQCGIT]); }
} else if (mode === 'metal') {
  ...
  if (!arg('part')) { const D = M.D();
    ...
    process.exit(0); }
  M.actId('hit', arg('part'));
  for (let j = 0; j < +(arg('secs') || 8); j++) { const ms = tick(50);
    console.log('t ' + t.toFixed(0) + ' fireEv kg ' + sc[SC_FIREKG].toFixed(2) + ' q ' + (sc[SC_FIREQ] / 1000).toFixed(1) + ' MJ pool ' + (s.roomPool.reduce((a, b) => a + b, 0) / 1000).toFixed(3) + ' t, ms ' + ms.toFixed(2)); }
} else if (mode === 'drain') {
  // the user's sequence: pour a deep pool on the deck, shoot the floor under it, then only wait
  M.actId('injectOn', 'fluid', 10000, at(29, 13));
  tick(200); M.act('injectOff');
  tick(100);
  M.actId('hit', 'mat:30,30');
  const q = water();
  let prev = null;
  for (let j = 0; j < 6; j++) { tick(75);
    ...
    for (let X = 7; X <= 36; X++) { let zz = null;
      for (let Y = 1; Y <= 29; Y++) { const i = at(X, Y), m = W()[i]; kg += m;
        if (!(m > 0.5) || M.eLqShut(i)) continue;
        if (M.eLqStands(q, i)) zz = M.eLqSurf(q, i); else air += m; }
    ...
    for (let X = 26; X <= 36; X++) { const i = at(X, 29);
      row += ' ' + X + ':' + (100 * W()[i] / Math.max(M.eLqCap(q, i), 1e-9)).toFixed(0) + (M.eLqStands(q, i) ? 'S' : 'a') +
  ...
  for (let j = 0; j < 2; j++) { tick(75);
    ...
    for (let X = 7; X <= 36; X++) { let zz = null, tt = null;
      for (let Y = GH - 1; Y >= 0; Y--) { const i = at(X, Y); if (W()[i] > 0.5 && M.eLqStands(q, i) && !M.eLqShut(i)) { zz = M.eLqSurf(q, i); tt = i; } }
      if (zz === null) continue;
  ...
  let over = 0; for (let i = 0; i < N; i++) over = Math.max(over, W()[i] - M.eLqCap(q, i));
  ...
  for (let j = 0; j < 175; j++) { tick(1); ticks++;
    ...
    for (let Y = 15; Y <= 24; Y++) for (let X = 7; X <= 36; X++) { if (X === 29) continue;
      const m = W()[at(X, Y)];
      if (m >= pxKg && m <= 0.1 * M.eLqCap(q, at(X, Y))) { if (Y <= 19) n1++; else n2++; } }
    let top = null;
    for (let Y = 0; Y < GH && top === null; Y++) { const i = at(29, Y); if (W()[i] > 0.5 && M.eLqStands(q, i) && !M.eLqShut(i)) top = Y; }
