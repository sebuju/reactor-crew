...
| MED | •••◦ | I | The axial xenon oscillation the bench warns of has never been seen | 23/09/26, `probe.js xeosc --secs=2000` (chunked, `--resume`) on the 100x clock: ~55 h, 1.9–3.7 real periods; AO settles at −44.6 % inside 50 s and holds to 0.16 pp for 2000 s, no swing, no sign change. Overdamped where a large PWR swings undamped. The premise "H/Lm 47, the unstable regime" stood on a 7.2 cm migration length the code does not compute (it reads 21 cm, row migration length), and the rest shape is the SOR solver's biased one (AO −44 %, `F_q` 3.29 on this core). Which of the mesh, the coupling law, `Lm` or the solver damps it is not tested | MODEL, cause unclassified | physics.md: Known gaps; fidelity: peaking factor `Fq`, migration length |
...
| HIGH | ••◦◦ | I | The flux solver settles on a biased shape, not the fundamental mode: in-place SOR renormalised every sweep. The stock PWR's `F_q` reads 2.878 against 2.152 on the true mode | diagnosed 23/09/26: uniform ρ with equal albedos reads 1.688 bottom against 1.319 top; Jacobi power iteration symmetric. Every reading off the shape carries it (axial offset, rod worth through φ², xenon, DNBR, `FqCold`); not measured on those | MODEL | fidelity: peaking factor `Fq` |
| MED | •◦◦◦ | I | The migration length is 21 cm where a water lattice's is 7–8 cm (`0.21·√` of a RELATIVE pitch) | ~2.8× long; stock `H/Lm` ~12, not 47 | MODEL | fidelity: migration length |
| LOW | ••◦◦ | I | The reflector is a per-node albedo, so its savings scale with the mesh | F_Z 1.215 against ~1.48, F_R 1.763 against ~2.07, ρ = 0 true mode; hand estimates | MODEL, unconfirmed | fidelity: the reflector's savings |
...
| HIGH | ••◦◦ | — | A sealed gas pocket is still squeezed past what the drive and the water can give it after a hot-leg break, and the pass is not named. The overflow walk is gone (23/09/26) and was not the pass: gas-free room in the water read a borrowed gas ring, and now reads its own p_sat or the gas beside it; a stiff cell's excess is a source in the solve | 23/09/26, energy judge (`roomgas.js breakhl`, 60 s armed): 56 of 2329 pocket-ticks over the Bagnold bound, worst 11.5× at 56.16 s (107.6 kJ against 9.3 kJ), all in the 6.6 MPa compartment pocket, no cavity reading at any of them; the stiff cells' over-cap volume is about a tenth of the pocket's loss. Fullest cell 1.0125 against 1.0112, FAIL; highest cell 12.65 MPa against an 11.81 MPa drive (a reading). Before this work, on the same tree with batch R's gas and plate terms: 80 over, fullest 1.0236, highest 27.97 MPa. `breaksl` 0 over | MODEL, pass not named | fidelity: ...water pushed into a full body |
