...
| MED | •••◦ | I | The axial xenon oscillation the bench warns of has never been seen | re-measured 23/09/26 on the exact solver and a real M (`probe.js xeosc --secs=2000 --every=25`, chunked): COMPACT at H/D 2.0 reads cz 0.066, **H/M 39**, inside the Randall–St John unstable regime a large PWR sits in. AO settles at −57.3 % and holds to 0.4 pp for 2000 s (~55 h on the 100x clock): no swing, no sign change, where a large PWR swings with a 15–30 h period (~30 h measured on a 900 MWe PWR, EPJ-N 2020, seen quoted). Solver bias and `Lm` are ruled out as the cause, and since 23/09/26 the rod-dominated rest too: with the banks at the bite (`ROD_BITE` 15/230) the same probe starts at AO −20.2 %, overshoots once to −22.3 % and settles at −21.4 % (F_q 3.16), still no sign change. Left: the 10-plane mesh (dz 29 cm, 3.9 M on a 2.89 m core), and the feedback weights | MODEL, cause unclassified | physics.md: Known gaps; fidelity: peaking factor `Fq` |
