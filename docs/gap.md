...
|---|---|---|---|---|---|---|
| HIGH | M | B | The face limiter squeezes sealed gas pockets: `eFaceLimit()` cuts a pocket's out-faces to the water its cells hold and leaves its in-faces, so the pocket takes volume the liquid solve never priced | 22/09/26, energy judge (`roomgas.js breakhl`): 102 of 2279 pocket-ticks over the Bagnold bound, worst 30.3× at 10.48 s (1722 kJ against 57 kJ); every probed violation at the limiter; highest cell 24.79 MPa against an 11.82 MPa drive (a reading); fullest cell 1.021 against 1.011, FAIL. `breaksl` 0 over | MODEL, pass named, unfixed (not one of batch B's three candidate passes) | fidelity: ...water pushed into a full body |
