...
|---|---|---|---|---|---|---|
| MED | ••◦◦ | — | The RBMK's hottest block runs over the published band | RBMK-1000 hottest block **886 C** against a 650–760 C band (126 K over) now its graphite stops 5.2 % of the heat (INSAG 5.5); its stack mean **554 C** against CAST's ~500 (PASS 15 %). The fuel channel's gas gap carries 223 of the fuel columns' 273 K mean rise, and the published ring gaps move it little; the stack is lumped per node and spreads no heat to its neighbours; no reflector cooling channels (`moderator.js s5`, 23/09/26) | MODEL (the channel count's ~11 K is BUILD) | fidelity: graphite temperature |
