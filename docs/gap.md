...
| HIGH | M | L | A snapshot restore on BWR/4 is not the run it came from | `holdPBy[2]` 6.8456 vs 6.8466 MPa 24 ticks after a restore; presets 0, 1 byte-identical; 3-8 not run | UNKNOWN, not traced | backlog 21/09/26 |
...
| LOW | S | A | The BLAST dial saturates at `ROOM_TMAX` | above 9.93 MPa on the dial (21/09/26, beside `pump0`) the source cell's constant-volume worth stops growing and the energy over it is discarded, so the dial does less than it says. The near field is not here: its distance from Baker is decision 8's unresolved front | MODEL | fidelity: what the BLAST fault injects |
...
| MEDIUM | M | L | A gas-charged tank's pressure and its mass are two independent tracks of one cushion | the level is read off the node's PRESSURE by the polytropic law (`eTankLvlA`) while the mass is what transport carried, and nothing forces them to agree. Measured 21/09/26 mid-blowdown on the STOCK PWR HPI accumulator at t = 2.00 s: the level says the gas swept to 20.356 m³, the delivered mass at 1000 kg/m³ says 20.067 m³ — **1.4 % apart in volume, which at n = 1.4 is the whole −5.0 % pressure distance** the accumulator check now reads (−4.2 % before batch G; that job moved it 0.8 pp by changing how fast the tank blows down, and either direction is the same defect). The same two-track shape as the circulating-water tank's 1.5e-6 booking residual | MODEL | fidelity: a tank's gas charge |
